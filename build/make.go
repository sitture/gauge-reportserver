...
var deployDir = filepath.Join(deploy, report)
...
func createDistro() {
	packageName := fmt.Sprintf("%s-%s-%s.%s", report, getPluginVersion(), getGOOS(), getArch())
...
func compileGoPackage(packageName string) {
	runProcess("go", "build", "-o", getGaugeExecutablePath(report))
}
...
func copyPluginFiles(destDir string) {
	...
	if getGOOS() == "windows" {
		files[filepath.Join(getBinDir(), report+".exe")] = bin
	} else {
		files[filepath.Join(getBinDir(), report)] = bin
	}
...
func installPlugin(installPrefix string) {
	...
	pluginInstallPath := filepath.Join(installPrefix, report, getPluginVersion())
