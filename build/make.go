...
var deployDir = filepath.Join(deploy, reportServer)
...
func compileGoPackage() {
	runProcess("go", "build", "-o", getGaugeExecutablePath(reportServer))
}
...
func copyPluginFiles(destDir string) {
	...
	if getGOOS() == "windows" {
		files[filepath.Join(getBinDir(), reportServer+".exe")] = bin
	} else {
		files[filepath.Join(getBinDir(), reportServer)] = bin
	}
...
func createDistro() {
	packageName := fmt.Sprintf("%s-%s-%s.%s", reportServer, getPluginVersion(), getGOOS(), getArch())
...
func installPlugin(installPrefix string) {
	...
	pluginInstallPath := filepath.Join(installPrefix, reportServer, getPluginVersion())
